...
include 'controllers/cittagiovani.controller.php';
include 'config/header.inc.php';
include 'config/navmenu.inc.php';


echo '</br>';
echo '<h1>Città per giovani (<18) '.$nazione.'</h1>';

echo '<div class="row">';
if (is_array($cittaGiovani) || is_object($cittaGiovani))
{
    foreach ($cittaGiovani as $item)
    {
        echo '<div class="col-md-3 d-flex align-items-stretch">';
        echo '<div class="card w-75 p-3 mt-4 mb-4 mx-auto">';
        echo '<a href="scuolegiovani.php?citta=' . $item->nome . '">';
        ...
        echo '<a href="scuolegiovani.php?citta=' . $item->nome . '" class="btn btn-primary">Scopri le scuole</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    }
